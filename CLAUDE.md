...
This is a **Spring Boot extension library** (not an application) that adds rollback functionality to Flyway Community Edition, similar to the `flyway undo` feature available in Flyway Enterprise. It is published via `publishToMavenLocal`; there is no `main()` to run.
...
- `RollbackAutoConfiguration` — Spring Boot auto-configuration entry, registered in `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`
