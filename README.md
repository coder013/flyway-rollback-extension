...
![Flyway Rollback Extension](docs/flyway_rollback_extension.png)
...
- Java 17+
- Spring Boot 3.x
- Flyway (provided by the user — `compileOnly`)
...
RollbackAutoConfiguration
    ...
    │               ├─ uses RollbackProperties         (reads target-version, dry-run)
    │               ├─ uses RollbackScriptLocator      (resolves R{v}__*.sql)
    │               ├─ uses SchemaHistoryRepository    (manipulates flyway_schema_history)
    │               └─ uses RollbackHistoryRepository  (records to flyway_rollback_history)
    ├─ creates RollbackHistoryRepository  (when history.enabled=true)
    └─ creates RollbackEndpoint           (when spring-boot-actuator is present)
```
...
├── RollbackMigrationStrategy        # 롤백 흐름을 오케스트레이션
├── RollbackScriptLocator            # classpath:db/rollback/ 에서 R{version}__*.sql 탐색
├── RollbackProperties               # flyway-extension.rollback.* 설정 바인딩
├── SchemaHistoryRepository          # flyway_schema_history 행 조회/삭제
├── RollbackHistoryRepository        # flyway_rollback_history 항목 기록
├── RollbackEndpoint                 # Spring Boot Actuator @Endpoint
└── exception/
    ├── RollbackScriptNotFoundException
    └── InvalidTargetVersionException
...
**왜 `FlywayMigrationStrategy`인가?**
이 빈이 등록되어 있으면 Spring Boot가 기본 `flyway.migrate()` 호출 대신 이 빈을 호출합니다. Flyway 내부를 패치하지 않는, 공식적으로 제공되는 확장 지점입니다.

**왜 사전 검증(fail-fast)을 하는가?**
롤백 도중 실패하면 DB가 중간 상태로 남습니다. DB를 건드리기 전에 모든 스크립트의 존재 여부를 먼저 확인함으로써 atomic한 실패를 보장합니다.

**왜 rollback 스크립트를 자동 생성하지 않는가?**
DDL을 자동으로 역연산하는 것은 본질적으로 불완전합니다. `ALTER TABLE ADD COLUMN`은 `DROP COLUMN`으로 되돌릴 수 있지만, 데이터가 있는 컬럼을 삭제하는 것은 비즈니스 결정입니다. 따라서 rollback 스크립트는 사용자가 직접 작성해야 합니다.
...
프로젝트에 `spring-boot-starter-actuator`를 추가하고 엔드포인트를 노출합니다.
